...
'''
def add_noise_to_parameters(clipped_update, indices, noise_std):
    # noise = []
    # for param in model.parameters():
    # size = 1024
    indices = indices.cpu()
    noise_std = noise_std.cpu()
    clipped_update_cpu = [param_diff.cpu() for param_diff in clipped_update]

    for param_diff in clipped_update_cpu:
        noise = torch.randn_like(param_diff)
        # Construct noise tensor
        if args.indices == "top":
            indices = torch.clamp(indices, 0, len(noise.view(-1)) - 1)
            top_noise = noise.view(-1)[indices].unsqueeze(1)  # Ensure it's a column tensor
            param_diff.view(-1).index_add_(0, indices, top_noise.view(-1) * noise_std)
        else:
            # Add noise to the bottom 20% parameters
            bottom_noise = noise.view(-1)[indices].unsqueeze(1)  # Ensure it's a column tensor
            param_diff.view(-1).index_add_(0, indices, bottom_noise * noise_std)
    clipped_update_gpu = [param_diff.to('cuda') for param_diff in clipped_update_cpu]
    return clipped_update_gpu
    # if args.indices == "top":
    #     # Add noise to the top 20% parameters
    #     for i in range(0, len(indices), size):
    #         batch_indices = indices[i:i + size]
    #         top_noise = noise.view(-1)[batch_indices].unsqueeze(1)
    #         param.view(-1)[batch_indices] = param.view(-1)[batch_indices] + top_noise.squeeze(1) * noise_std
    # else:
    #     # Add noise to the bottom 20% parameters
    #     for i in range(0, len(indices), size):
    #         batch_indices = indices[i:i + size]
    #         bottom_noise = noise.view(-1)[batch_indices].unsqueeze(1)
    #         param.view(-1)[batch_indices] = param.view(-1)[batch_indices] + bottom_noise * noise_std
    # return clipped_update
'''


def add_noise_to_parameters(clipped_update, indices, noise_std):
    # Flatten the clipped_update to align with flattened_fisher_diag
    flattened_clipped_update = torch.cat([param_diff.view(-1).cpu() for param_diff in clipped_update])

    # Construct noise tensor
    noise = torch.randn_like(flattened_clipped_update)

    # Ensure indices are within bounds
    indices = torch.clamp(indices.cpu(), 0, len(flattened_clipped_update) - 1)

    # Add noise to the selected parameters
    selected_noise = noise[indices].unsqueeze(1)
    flattened_clipped_update.index_add_(0, indices, selected_noise.view(-1) * noise_std)

    # Reshape back to the original shape
    start = 0
    for param_diff in clipped_update:
        end = start + param_diff.numel()
        param_diff.data = flattened_clipped_update[start:end].view(param_diff.shape)
        start = end

    # Move the result back to GPU
    clipped_update_gpu = [param_diff.to('cuda') for param_diff in clipped_update]
...
def main():
    ...
    for epoch in trange(global_epoch):
        ...
        noisy_updates = []
        for idx, (client_update, fisher_diag) in enumerate(zip(clients_model_updates, fisher_diags)):
            if not args.no_clip:
            ...
            flattened_fisher_diag = torch.cat([fisher.view(-1) for fisher in fisher_diag])
            if args.indices == "top":
                flattened_fisher_diag_cpu = flattened_fisher_diag.cpu()
                _, indices = torch.topk(flattened_fisher_diag_cpu, int(args.partition * len(flattened_fisher_diag_cpu)))
                # _, indices = torch.topk(flattened_fisher_diag, int(args.partition * len(flattened_fisher_diag)))
            else:
                indices = torch.argsort(flattened_fisher_diag)[:int(args.partition * len(flattened_fisher_diag))]

            noise_stddev = torch.sqrt(torch.tensor((clipping_bound ** 2) * (noise_multiplier ** 2) / num_clients))

            # noisy_update = [add_noise_to_parameters(clipped_update, indices, noise_stddev)]
            noisy_updates.append(add_noise_to_parameters(clipped_update, indices, noise_stddev))
    ...
    directory_path = './logs/fisher_compare/fisher_partition_eps_{}'.format(target_epsilon)
    os.makedirs(directory_path, exist_ok=True)
    with open(directory_path + '/{}_{}_{}.txt'.format(dataset, args.indices, args.partition), 'a') as temp_file:
