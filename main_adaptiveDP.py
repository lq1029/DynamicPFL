...
def main():
    ...
    for epoch in trange(global_epoch):
        ...
        for idx, (client_model, client_trainloader, client_testloader) in enumerate(
            ...
            for param, fisher_value in zip(client_model.parameters(), fisher_diag):
                ...
                # 概率取1
                mask[fisher_value >= fisher_diag_threshold] = mask[fisher_value >= fisher_diag_threshold].bernoulli_(
                    1 - filter_rate)
                # 按照比例过滤出不需要添加噪声的参数，生成的filter_params为最终的过滤矩阵
                filter_params.append(torch.ones_like(param) * (1 - mask))

            clients_filter_params.append(filter_params)
        ...
        # clipped & noise add
        for idx, (client_update, filter_params) in enumerate(zip(clients_model_updates, clients_filter_params)):
            if not args.no_clip:
            ...
            noise = [torch.randn_like(filter_param) * noise_stddev * filter_param for filter_param in filter_params]
    ...
    with open(directory_path + '/adaptiveDP.txt', 'a') as temp_file:
        temp_file.write(f"mean_acc{mean_acc_s}\n"
                        f"mean_loss{mean_loss_s}")

    # directory_path = './logs/{}_eps_{}_fisher_{}_filter_rate'.format(dataset, target_epsilon,
    #                                                                  args.fisher_threshold)
    # os.makedirs(directory_path, exist_ok=True)
    # with open(directory_path + '/adaptiveDP_filter_rate_{}.txt'.format(args.filter_rate), 'a') as temp_file:
    #     temp_file.write(f"mean_acc{mean_acc_s}\n"f"mean_loss{mean_loss_s}")
